...
export interface HomePageData {
  navigation: NavigationItem[]; marquees: MarqueeItem[]; latestJobs: JobEntry[];
  results: ResultEntry[]; admitCards: AdmitCardEntry[]; answerKeys: AnswerKeyEntry[];
  syllabus: SyllabusEntry[]; admissions: AdmissionEntry[]; importantLinks: ImportantLink[]; lastUpdated: string
}
export interface ScrapedContent { html: string; text: string; title?: string; url: string }
export interface ApiResponse<T> { success: boolean; data?: T; error?: string; cached?: boolean; timestamp: string }
